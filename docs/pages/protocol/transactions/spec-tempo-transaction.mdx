...
rlp([
    ...
    signature            // PrimitiveSignature bytes (secp256k1 only for root key signatures)
])
```
...
The `signature` is a `PrimitiveSignature` (secp256k1 only) signed by the root key over the `key_authorization_digest`.
...
2. **Create Authorization Message**
   ...
   const keyAuth = {
     chain_id: 42429,                   // Tempo chain ID (0 = valid on any chain)
...
4. **Build TempoTransaction**
   ...
   > **Note:** When RLP encoding for `eth_sendRawTransaction`, the `key_authorization` field is encoded as a nested list: `rlp([rlp([chain_id, key_type, key_id, expiry?, limits?]), signature])`. The `key_authorization` comes **after** `aa_authorization_list` in the RLP field order.
