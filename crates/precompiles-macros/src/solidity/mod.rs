...
pub(crate) fn expand(item: ItemMod, config: SolidityConfig) -> syn::Result<TokenStream> {
    ...
    // Generate Interface trait and Calls enum (user-defined or dummy)
    let (interface_impl, calls_impl) = if let Some(ref def) = module.interface {
        (
            Some(interface::generate_interface(def, &registry)?),
            None, // Calls is generated by generate_interface
        )
    } else {
        (None, Some(common::generate_dummy_container(AbiType::Calls)))
    };
