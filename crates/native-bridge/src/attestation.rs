...
impl PartialSignature {
    pub const fn new(index: u32, signature: [u8; G2_COMPRESSED_LEN]) -> Self {
...
impl AggregatedSignature {
    pub const fn new(signature: [u8; G2_COMPRESSED_LEN], epoch: u64) -> Self {
...
mod signature_bytes {
    ...
    pub(super) fn serialize<S>(bytes: &[u8; 96], serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
    ...
    pub(super) fn deserialize<'de, D>(deserializer: D) -> Result<[u8; 96], D::Error>
    where
        D: Deserializer<'de>,
    {
